...
class AESGCMEngine(EncryptionInterface):
    ...
    def __init__(
        ...
        self._log_security_event("engine_initialized", {
    ...
    def encrypt(
        ...
        try:
            ...
            encryption_key = key_data["key_bytes"]
            ...
            self._log_security_event("encryption_success", {
            ...
            error_msg = f"Encryption failed: {type(e).__name__}"
            self._log_security_event("encryption_failed", {
                "error_type": type(e).__name__,
                "key_id": target_key_id if 'target_key_id' in locals() else None
            })
    ...
    def decrypt(
        ...
        try:
            ...
            self._log_security_event("decryption_success", {
    ...
    def rotate_key(self, old_key_id: str) -> str:
        ...
        try:
            ...
            old_current = self._current_key_id
    ...
    def _check_key_rotation_needed(self, key_id: str) -> None:
        ...
        if datetime.utcnow() >= key_data["rotation_due"]:
            self._log_security_event("key_rotation_due", {
                "key_id": key_id,
                "reason": "time_based"
    ...
    def _log_security_event(
        ...
        safe_details = {k: v for k, v in details.items() if k not in ["key_bytes", "plaintext", "password"]}
        event["details"] = safe_details

        self._logger.log(level, f"AES-GCM Security Event: {event_type}", extra={"security_event": event})
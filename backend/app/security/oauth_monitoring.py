...
class OAuthSecurityMonitor:
    ...
    def _assess_threat_level(
        ...
        # High threats
        if event_type == "invalid_redirect_uri":
            return ThreatLevel.HIGH

        if event_type == "pkce_violation" or event_type == "state_violation":
            return ThreatLevel.HIGH
        ...
        if self._has_suspicious_patterns(source_ip, details):
            return ThreatLevel.MEDIUM
    ...
    def _count_recent_failures(self, source_ip: str, minutes: int) -> int:
        ...
        for event in self.events_short:
            if (event.source_ip == source_ip and
                ...
                event.event_type in ["oauth_callback", "token_validation"] and
                "failure" in event.details.get("status", "")):
                failures += 1
    ...
    def _has_suspicious_patterns(self, source_ip: str, details: Dict[str, Any]) -> bool:
        ...
        recent_requests = len([
            event for event in self.events_short
            if (event.source_ip == source_ip and
    ...
    def _check_immediate_threats(self, event: SecurityEvent):
        ...
        if self._check_rate_limit_violation(event.source_ip):
            self._send_alert(
                ...
                {"source_ip": event.source_ip, "recent_requests": self._count_recent_requests(event.source_ip)}
            )
    ...
    def _check_rate_limit_violation(self, source_ip: str) -> bool:
        ...
        recent_requests = len([
            ts for ts in self.ip_activity[source_ip]
    ...
    def _analyze_request_patterns(self):
        ...
        # Find anomalous IPs
        threshold = mean_requests + (self.thresholds["anomaly_detection_threshold"] * stdev_requests)
    ...
    def _analyze_user_agent_patterns(self):
        ...
        for event in self.events_medium:
            if (event.timestamp >= hour_ago and
                event.user_agent and
                len(event.user_agent) > 10):  # Filter out short/empty user agents
        ...
        for user_agent, count in user_agent_counts.items():
            if count > self.thresholds["suspicious_user_agent_threshold"]:
                self._send_alert(
                    ...
                    ThreatLevel.MEDIUM,
                    f"Suspicious user agent pattern detected",
                    {
        ...
        if len(hourly_counts) >= 3:
            ...
            if current_hour_count > mean_hourly * 5 and current_hour_count > 50:
                self._send_alert(
                    AlertType.AUTHENTICATION_ANOMALY,
                    ThreatLevel.MEDIUM,
                    f"Unusual spike in OAuth activity",
                    {
            ...
            try:
                for provider, health in self.provider_health.items():
                    ...
                    if health["last_success"]:
                        ...
                        if time_since_success > timedelta(hours=1) and health["status"] == "healthy":
                            health["status"] = "degraded"
                            self._send_alert(
                                AlertType.PROVIDER_AVAILABILITY,
                                ThreatLevel.MEDIUM,
                                f"OAuth provider {provider} has not had successful authentication in over 1 hour",
                                {"provider": provider, "time_since_success": str(time_since_success)}
                            )
            ...
            try:
                ...
                for ip in list(self.ip_activity.keys()):
                    ...
                    self.ip_activity[ip] = deque([
                        ts for ts in self.ip_activity[ip] if ts >= cutoff_time
                    ], maxlen=100)
    ...
    def _send_external_alert(self, alert_data: Dict[str, Any]):
        ...
        if alert_data["threat_level"] in ["high", "critical"]:
            self.logger.critical(
                f"SECURITY ALERT: {alert_data['message']}",
                extra=alert_data
            )
            ...
            self.logger.warning(
                f"Security notification: {alert_data['message']}",
                extra=alert_data
            )
    ...
    def get_security_metrics(self, timeframe_hours: int = 24) -> SecurityMetrics:
        ...
        if timeframe_hours <= 1:
            events = self.events_short
        elif timeframe_hours <= 24:
            events = self.events_medium
        else:
            events = self.events_long
        ...
        for event in events:
            if event.timestamp >= cutoff_time:
                ...
                elif event.event_type == "invalid_redirect_uri":
                    metrics.invalid_redirect_attempts += 1
                elif event.event_type == "token_validation" and not event.details.get("success"):
                    metrics.token_validation_failures += 1
                elif event.event_type == "pkce_violation":
                    metrics.pkce_failures += 1
                elif event.event_type == "state_violation":
                    metrics.state_validation_failures += 1
...
def get_oauth_security_dashboard() -> Dict[str, Any]:
    ...
    return {
        ...
        "metrics_last_24h": {
            ...
            "success_rate": (metrics_24h.oauth_successes / max(metrics_24h.oauth_attempts, 1)) * 100,
...
def _calculate_overall_threat_level(metrics_1h: SecurityMetrics, metrics_24h: SecurityMetrics) -> str:
...
def _get_security_recommendations(metrics_1h: SecurityMetrics, metrics_24h: SecurityMetrics) -> List[str]:
    ...
    if metrics_1h.oauth_attempts > 0:
        failure_rate = metrics_1h.oauth_failures / metrics_1h.oauth_attempts
        if failure_rate > 0.3:
            recommendations.append("High OAuth failure rate detected. Review authentication flows and provider configurations.")

    # Rate limiting
    if metrics_1h.rate_limit_violations > 2:
        recommendations.append("Multiple rate limit violations detected. Consider tightening rate limits or investigating automated attacks.")

    # Provider errors
    if any(count > 5 for count in metrics_24h.provider_errors.values()):
        recommendations.append("High error rate with OAuth providers. Check provider status and credential validity.")

    # Suspicious activity
    if metrics_1h.suspicious_patterns_detected > 1:
        recommendations.append("Suspicious OAuth activity detected. Review security logs and consider additional monitoring.")

    if not recommendations:
        recommendations.append("OAuth security metrics are within normal ranges. Continue monitoring.")

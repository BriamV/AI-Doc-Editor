...
async def store_user_credentials(
    credentials: UserCredentials, current_user: User = Depends(get_current_user)
):
    """
    ...
    return UserCredentialsResponse(has_api_key=True, key_preview=key_preview)


@router.get("/credentials", response_model=UserCredentialsResponse)
async def get_user_credentials_status(current_user: User = Depends(get_current_user)):
    """
...
async def delete_user_credentials(current_user: User = Depends(get_current_user)):
    """
    ...
    raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="No API key found for user")

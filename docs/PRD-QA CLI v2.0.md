...
- **Design Metrics**: Métricas de calidad de código evaluadas con un sistema de semáforos para proporcionar una guía flexible y accionable. Los umbrales serán configurables a través de los archivos nativos de cada herramienta (ej. `eslint.config.js`, `ruff.toml`).
...
**ARQUITECTURA REVISADA (v2.0):** Orquestador inteligente con herramientas directas optimizadas por stack tecnológico. Basado en ADR-009.

> **⚠️ DOCUMENTO FUENTE DE VERDAD**: Este PRD v2.0 es la **única fuente de verdad** para todos los requisitos del sistema QA. Aunque existen artefactos técnicos intermedios (`.mega-linter.yml`, configs, etc.), toda implementación y validación debe realizarse **exclusivamente contra este PRD**. Los archivos `.mega-linter.yml` y `.mega-linter.yml.backup` serán **descontinuados** post-migración.

| Dimensión de Validación (RF-003) | Herramienta Principal | Stacks Cubiertos | Configuración y Notas Clave |
| :--- | :--- | :--- | :--- |
| **Error Detection** (Linting y Formato) | **Direct Linters** (ESLint flat config, Prettier, Ruff, Black, markdownlint-cli2, yamllint, ShellCheck) | Frontend (TS/React), Backend (Python), Docs (.md), Config (.yml, .json), Tooling (.cjs, .sh) | **Configuración migrada**: Reutilizar reglas de `.mega-linter.yml` en configuraciones existentes (`eslint.config.js`, `pyproject.toml`, `.pylintrc`) y crear configs faltantes según necesidad. |
...
| **Design Metrics** (Complejidad, LOC) | **Direct Linters** (ESLint complexity rules, Ruff complexity checks) | Frontend (TS/React), Backend (Python) | **Complejidad Ciclomática:** 🟢(≤10), 🟡(11-15), 🔴(>15). <br> **LOC por Archivo:** 🟢(≤212), 🟡(213-300), 🔴(>300). <br> **Longitud de Línea:** Límite estricto de 100 caracteres. Performance nativa vs overhead Docker. |

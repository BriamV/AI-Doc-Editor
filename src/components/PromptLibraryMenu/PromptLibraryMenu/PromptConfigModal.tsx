...
export const PromptConfigModal = ({
  ...
  const handleIndividualPromptConfig = () => {
    if (prompt.config == null) {
      const tempPrompts = _prompts;
      const tempPromptConfig = _defaultChatConfig;
  ...
  return (
    <PopupModal title={'Prompt Config'} setIsModalOpen={setIsModalOpen} fullWidth={true}>
      <div className="p-6 border-b border-gray-200 dark:border-gray-600 w-full">
        ...
        {prompt.config == null ? null : <PromptConfigPanel prompt={prompt} index={index} />}
      </div>
    </PopupModal>
  );
};

...
// Custom hook for callback processing
const useCallbackProcessor = (onSuccess?: () => void, onError?: (error: string) => void) => {
...
const AuthCallback: React.FC<AuthCallbackProps> = ({ onSuccess, onError }) => {
  const { status, errorMessage } = useCallbackProcessor(onSuccess, onError);

  if (status === 'processing') return <ProcessingStatus />;
  if (status === 'success') return <SuccessStatus />;
  if (status === 'error') return <ErrorStatus errorMessage={errorMessage} />;
  return null;
};

...
class VenvManager {
  ...
  detectVirtualEnvironment() {
    const possibleVenvPaths = [
      path.join(this.projectRoot, '.venv'),
      path.join(this.projectRoot, 'venv'),
      path.join(this.projectRoot, '.virtualenv'),
    ...
    for (const venvPath of possibleVenvPaths) {
      if (this._isValidVenv(venvPath)) {
        ...
        this.logger.info(`📦 Virtual environment detected: ${path.relative(this.projectRoot, venvPath)}`);
        return true;
      }
    }
  ...
  _isValidVenv(venvPath) {
    ...
    const pythonPath = this._getVenvPythonPath(venvPath);
    const activateScript = this._getActivateScript(venvPath);
    
    return fs.existsSync(pythonPath) || fs.existsSync(activateScript);
  }

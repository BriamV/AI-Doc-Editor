...
class BuildResultBuilder {
  ...
  _determineOverallSuccess(processedResults) {
    ...
    // Additional check: If there are environment validation errors, consider it a failure
    const hasEnvErrors = processedResults.some(result => 
      result.errors && result.errors.some(error => 
        typeof error === 'string' && error.includes('Environment validation failed')
      )
    );
    if (hasEnvErrors) {
      return false;

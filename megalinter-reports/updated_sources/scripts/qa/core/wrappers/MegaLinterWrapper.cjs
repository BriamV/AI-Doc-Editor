...
class MegaLinterWrapper {
  ...
  async _prepareEnvironment(tool) {
    ...
    // Apply mode-specific optimizations (SOLID: scalable for all modes)
    const modeConfig = this.megalinterConfig.getModeConfig(tool.config.mode);
    Object.assign(envVars, modeConfig);
    
    // Apply fast mode file filtering

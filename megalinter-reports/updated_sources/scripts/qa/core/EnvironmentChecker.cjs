...
class EnvironmentChecker {
  constructor(config, logger) {
    ...
    this.optionalTools = {
      megalinter: { command: 'npm list -g mega-linter-runner --depth=0', description: 'MegaLinter (local installation) - EXCEPTION: uses npm not yarn due to --version issues', installUrl: 'https://megalinter.github.io/latest/installation/', fallback: 'docker' },
      ...
      prettier: { command: 'yarn exec prettier --version', description: 'Prettier code formatter', installUrl: 'lazy:prettier', fallback: 'megalinter' },
      eslint: { command: 'yarn exec eslint --version', description: 'ESLint JavaScript linter', installUrl: 'lazy:eslint', fallback: 'megalinter' },
  ...
  async checkEnvironment() {
    ...
    if (venvDetected) {
      ...
      this.logger.info(`🐍 Virtual environment detection: true`);
    } else {
      this.logger.info('🔍 No virtual environment detected - using system Python');
      this.logger.info(`🐍 Virtual environment detection: false`);
    }
    ...
    try {
      ...
      Object.entries(this.optionalTools).forEach(([name, config]) => {
        if (['black', 'pylint', 'pytest'].includes(name)) {
      ...
      const allOptionalTools = { ...optionalRequired, ...npmOptionalTools, ...pythonOptionalTools };
      const detectionResults = await this.sharedToolService.detectAllTools(this.requiredTools, allOptionalTools);

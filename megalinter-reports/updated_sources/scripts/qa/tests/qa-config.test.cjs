...
describe('QAConfig', () => {
  ...
  test('should validate Design Metrics thresholds (RF-003)', () => {
    const config = new QAConfig();
    ...
    expect(config.get('limits.maxComplexity')).toBe(10);
    
    // Test validation of these limits
    const validation = config.validate();
    expect(validation.isValid).toBe(true);
  });
  
  test('should provide DoD mappings (RF-005)', () => {
    const config = new QAConfig();
    
    // Test dod:code-review mapping
    const codeReviewMapping = config.getDoDMapping('code-review');
    expect(codeReviewMapping).toEqual(['format', 'lint', 'security']);
    
    // Test dod:all-tests mapping
    const testingMapping = config.getDoDMapping('testing');
    expect(testingMapping).toEqual(['test', 'coverage']);
  });
  
  test('should validate context to dimensions mapping', () => {
    const config = new QAConfig();
    
    // Test that branch types map to appropriate dimensions
    const featureBranchDimensions = config.getDimensionsForContext('feature');
    expect(Array.isArray(featureBranchDimensions)).toBe(true);
    
    // Should include error detection for feature branches
    expect(featureBranchDimensions).toContain('format');
  });
});
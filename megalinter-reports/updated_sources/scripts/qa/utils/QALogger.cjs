...
class QALogger {
  ...
  /**
   * Display structured violation details for enhanced MegaLinter output
    ...
    for (const detail of details) {
      ...
      if (detail.level === 'infrastructure') {
        for (const item of detail.items) {
          ...
          console.log(`${timestamp}${indent}🔧 ${this.colorize(item.message, 'yellow')}`);
          
          // Actionable suggestion with different icon
          if (item.suggestion) {
            console.log(`${timestamp}${this.getIndentation()}  💡 ${this.colorize(item.suggestion, 'cyan')}`);
          }
        }
      }
      ...
      // Code quality violations (grouped by file)
        ...
        for (const fileGroup of detail.items) {
          ...
          // File header
          console.log(`${timestamp}${indent}📁 ${this.colorize(fileGroup.file, 'bright')}`);
          ...
          for (const violation of fileGroup.violations) {
            ...
            // Violation details
            console.log(`${timestamp}${violationIndent}├─ ${this.colorize(violation.rule, 'red')}${locationInfo}`);
            console.log(`${timestamp}${violationIndent}│  ${this.colorize(violation.message, 'gray')}`);
            
            if (violation.suggestion) {
              console.log(`${timestamp}${violationIndent}└─ 💡 ${this.colorize(violation.suggestion, 'cyan')}`);
            }
          }
          this.currentLevel--;
        }
      ...
      // Summary information
        ...
        for (const item of detail.items) {
          ...
          console.log(`${timestamp}${indent}📊 ${this.colorize(item.message, 'blue')}`);
          if (item.suggestion) {
            console.log(`${timestamp}${this.getIndentation()}  💡 ${this.colorize(item.suggestion, 'cyan')}`);
          }
        }
  ...
  _displayEnhancedErrorContext(toolName, results) {
    ...
    console.log(`${timestamp}${baseIndent}  🔧 ${this.colorize(errorContext.category, 'yellow')}`);
    
    // Display primary message
    if (errorContext.message) {
      console.log(`${timestamp}${baseIndent}  💡 ${this.colorize(errorContext.message, 'cyan')}`);
    }
    
    // Display actionable suggestions
    if (errorContext.suggestions && errorContext.suggestions.length > 0) {
      console.log(`${timestamp}${baseIndent}  📋 ${this.colorize('Possible solutions:', 'bright')}`);
      errorContext.suggestions.forEach((suggestion, index) => {
        console.log(`${timestamp}${baseIndent}    ${index + 1}. ${suggestion}`);
      });
    }
  }
  ...
  async generateJSONReport(results, format = 'json') {
    ...
    if (format === 'ci-json') {
      ...
      const context = {
        ...
        megalinterReports: 'megalinter-reports/'
      };
